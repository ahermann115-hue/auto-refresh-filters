...
# Фильтруем ТОЛЬКО реальные домены
grep '^0\.0\.0\.0[[:space:]]' raw_combined.txt | \
    awk '{
        # Берем второй столбец (после 0.0.0.0)
        domain = $2
        
        # Удаляем комментарии если есть
        sub(/#.*$/, "", domain)
        
        # Удаляем "*.", если домен начинается с него
        sub(/^\*\./, "", domain)
        
        # Проверяем что это валидный домен (не localhost, не IP адрес)
        if (domain ~ /^[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}/ && 
            domain !~ /^(localhost|broadcasthost|ip6-|local|0\.0\.0\.0|127\.0\.0\.1|255\.255\.255\.255)$/) {
            print domain
        }
    }' | sort -u > domains.txt

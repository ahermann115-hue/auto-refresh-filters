...
# 2. Очищаем AdBlock формат - ТЕПЕРЬ БОЛЕЕ АГРЕССИВНО
echo "🧹 Очищаем формат AdBlock (усиленная очистка)..."
# Убираем ВСЕ символы AdBlock формата
cat oisd_raw.txt | \
    sed -e 's/^\|\|//' \
        -e 's/\^$//' \
        -e 's/^|http:\/\///' \
        ...
        -e 's/^\*\.//' \
        -e 's/^\.//' | \
    # Убираем всё, что не похоже на домен

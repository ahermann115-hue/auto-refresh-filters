...
# Берем ТОЛЬКО строки, начинающиеся с 0.0.0.0
grep '^0\.0\.0\.0' raw_combined.txt | \
    # Удаляем "0.0.0.0" из начала строки
    sed 's/^0\.0\.0\.0[[:space:]]*//' | \
    # Удаляем комментарии после домена
    sed 's/[[:space:]]*#.*$//' | \
    # Удаляем ведущие "*.", если есть
    sed 's/^\*\.//' | \
    # Удаляем пустые строки и строки, которые стали пустыми после удаления
    grep -v '^[[:space:]]*$' | \
    # Удаляем строки, которые остались IP адресами (0.0.0.0, 127.0.0.1 и т.д.)
    grep -v -E '^(0\.0\.0\.0|127\.0\.0\.1|255\.255\.255\.255|[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+)$' | \
    # Берем только то, что выглядит как домен

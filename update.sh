...
/^0\.0\.0\.0[[:space:]]/ {
    ...
    # РЕКУРСИВНО удаляем "0.0.0.0." в начале
    while (domain ~ /^0\.0\.0\.0\./) {
        sub(/^0\.0\.0\.0\./, "", domain)
    }
    ...
    # Проверяем что это не IP адрес и не служебная запись
    if (domain != "" && 
        domain !~ /^(0\.0\.0\.0|127\.0\.0\.1|255\.255\.255\.255)$/ &&
        domain !~ /^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$/ &&
        domain ~ /\./) {  # Должна быть хотя бы одна точка
        print domain
    }
}
' raw_combined.txt | sort -u > domains.txt
